...
from gates.pzy1 import paeyezy
...
@Client.on_message(filters.command(['nq'], PREFIXES))
async def gate_te(client, message):
    ...
    gate_info = GateManager.get_gate_info("/nq")
...
{symbol} Última Revisión: {gate_info['ultima_revision']}</b>""", reply_to_message_id=message.id)
    ...
    msgedit = await client.send_message(
        ...
        text=f"""
<b>{symbol} Gateway: Payeezy Charged</b> (/nq) 
...
<b>{symbol} Took: {taken2}</b>
    ...
    while retries < max_retries:
        try:
            ...
            status, response = await paeyezy(cc, mes, ano, cvv, await ProxyRandom())
...
{symbol} Gateway: Payeezy <code>[CHARGED]</code>
