...
from gates.moneris import mn
from functions.functions import GetCC, Symbol, get_bin_info, AntiSpam, bright_data_proxy, proxy_x
...
@Client.on_message(filters.command(['nx'], PREFIXES))
async def gate_nx(client, message):
...
{symbol} Última Revisión: <code>{gate_info['ultima_revision']}</code></b>""", reply_to_message_id=message.id)
    ...
    msgedit = await client.send_message(
        ...
        text=f"""
<b>{symbol} Gateway: Moneris <code>[AUTH]</code> </b> (/nx) 
...
<b>{symbol} Took: {taken2}</b>
    ...
    while retries < max_retries:
        try:
            ...
            status, response = await mn(cc, mes, ano, cvv,)
...
{symbol} Gateway: Moneris <code>[AUTH]</code>
