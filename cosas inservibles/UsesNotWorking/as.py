...
from gates.ar import fullsteam
...
@Client.on_message(filters.command(['as'], PREFIXES))
async def gate_te(client, message):
    ...
    gate_info = GateManager.get_gate_info("/as")
    
    # Verificar estado del gate con la información más reciente
    if gate_info["estado"] == "OFF":
        return await message.reply_text(f"""♯𝗭𝘆𝗿𝗲𝘅 𝗖𝗵𝗸 | UsaEpay CCN (/as)
━━━━━━━━━━━━━━━━━━━━
...
""", reply_to_message_id=message.id)
    ...
    msgedit = await client.send_message(
        ...
        text=f"""
<b>{symbol} Gateway: UsaEpay CCN</b> (/as) 
...
<b>{symbol} Took: {taken2}</b>
    ...
    while retries < max_retries:
        try:
            ...
            status, response = await fullsteam(cc, mes, ano, cvv, await ProxyRandom())
...
{symbol} Gateway: UsaEpay CCN <code>[CCN]</code>
