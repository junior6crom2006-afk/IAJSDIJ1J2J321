...
from gates.chargify import chargify
...
async def gate_te(client, message):
    ...
    gate_info = GateManager.get_gate_info("/sa")
...
{symbol} Última Revisión: {gate_info['ultima_revision']}</b>""", reply_to_message_id=message.id)
    ...
    msgedit = await client.send_message(
        ...
        text=f"""
<b>{symbol} Gateway: Chargify Braintree AVS</b> (/sa) 
...
<b>{symbol} Took: {taken2}</b>
    ...
    while retries < max_retries:
        try:
            ...
            status, response = await chargify(cc, mes, ano, cvv, await ProxyRandom())
...
{symbol} Gateway: Chargify Braintree <code>[AUTH]</code>
