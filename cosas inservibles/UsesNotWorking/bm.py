...
from gates.avsrejected import bm_auth
...
@Client.on_message(filters.command(['bm'], PREFIXES))
async def gate_te(client, message):
    ...
    gate_info = GateManager.get_gate_info("/bm")
...
{symbol} Última Revisión: {gate_info['ultima_revision']}</b>""", reply_to_message_id=message.id)
    ...
    msgedit = await client.send_message(
        ...
        text=f"""
<b>{symbol} Gateway: Unknown CCN</b> (/bm) 
...
<b>{symbol} Took: {taken2}</b>
    ...
    while retries < max_retries:
        try:
            ...
            status, response = await bm_auth(cc, mes, ano, cvv, await ProxyRandom())
...
{symbol} Gateway: Unknown CCN <code>[CCN]</code>
