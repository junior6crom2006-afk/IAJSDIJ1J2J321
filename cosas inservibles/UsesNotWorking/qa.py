...
from gates.vantiv import vantivccn
from functions.functions import GetCC, Symbol, get_bin_info, AntiSpam, ProxyRandom, proxy_x, ProxyRandomKev
...
@Client.on_message(filters.command(['qa'], PREFIXES))
async def gate_te(client, message):
    ...
    gate_info = GateManager.get_gate_info("/qa")
...
<b>{symbol} Took: {taken2}</b>
    ...
    while retries < max_retries:
        try:
            ...
            status, response = await vantivccn(cc, mes, ano, cvv, await ProxyRandom())
...
{symbol} Gateway: Vantiv <code>[CCN]</code>
