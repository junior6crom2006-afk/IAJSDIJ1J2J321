...
from gates.adyen1 import adyenauth
...
@Client.on_message(filters.command(['rag'], PREFIXES))
async def gate_te(client, message):
    ...
    gate_info = GateManager.get_gate_info("/rag")
...
{symbol} Última Revisión: {gate_info['ultima_revision']}</b>""", reply_to_message_id=message.id)
    ...
    msgedit = await client.send_message(
        ...
        text=f"""
<b>{symbol} Gateway: Adyen Auth</b> (/rag) 
...
<b>{symbol} Took: {taken2}</b>
    ...
    while retries < max_retries:
        try:
            ...
            status, response = await adyenauth(cc, mes, ano, cvv, await ProxyRandom())
...
{symbol} Gateway: Adyen <code>[AUTH]</code>
