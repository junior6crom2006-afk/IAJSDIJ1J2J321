...
@Client.on_message(filters.command(['xd'], PREFIXES))
async def gate_xd(client, message):
    ...
    msgedit = await client.send_message(
        ...
        text=f"""
<b>{symbol} Gateway: Stripeccn1 </b> (/kl) 
...
<b>{symbol} Took: {taken2}</b>
    ...
    while retries < max_retries:
        try:
            ...
            status, response = await stripeccn1(cc, mes, ano, cvv, await ProxyRandom())
...
{symbol} Gateway: Stripe <code>[CCN]</code>
