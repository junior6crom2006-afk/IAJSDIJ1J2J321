...
from gates.chaseccn import chaseccn1
...
@Client.on_message(filters.command(['te'], PREFIXES))
...
{symbol} Última Revisión: <code>{gate_info['ultima_revision']}</code></b>""", reply_to_message_id=message.id)
    ...
    msgedit = await client.send_message(
        ...
        text=f"""
<b>{symbol} Gateway: Chase CCN_AUTH </b> (/te) 
...
<b>{symbol} Took: {taken2}</b>
    ...
    while retries < max_retries:
        try:
            ...
            status, response = await chaseccn1(cc, mes, ano, cvv, await ProxyRandom())
...
{symbol} Gateway: Chase <code>[CCN_AUTH]</code>
