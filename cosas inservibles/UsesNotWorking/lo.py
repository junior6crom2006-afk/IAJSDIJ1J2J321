...
from gates.adyen import rapidotc
...
@Client.on_message(filters.command(['lo'], PREFIXES))
async def gate_te(client, message):
    ...
    gate_info = GateManager.get_gate_info("/lo")
...
<b>{symbol} Took: {taken2}</b>
    ...
    while retries < max_retries:
        try:
            ...
            status, response = await rapidotc(cc, mes, ano, cvv, await ProxyRandom())
...
{symbol} Gateway: Adyen <code>[CCN]</code>
