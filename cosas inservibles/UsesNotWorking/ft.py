...
from gates.zuora import ch
from functions.functions import GetCC, Symbol, get_bin_info, AntiSpam, bright_data_proxy, proxy_x, Symbol2
...
@Client.on_message(filters.command(['ft'], PREFIXES))
async def gate_ft(client, message):
...
<b>{symbol} Took: {taken2}</b>
    ...
    while retries < max_retries:
        try:
            ...
            status, response,avs,cvv2 = await ch(cc, mes, ano, cvv,)
...
{symbol2} AVS: <code>{avs}</code> CVV2: <code>{cvv2}</code>
{symbol} Gateway: Zuora Chase<code>[Auth]</code>
