...
@Client.on_message(filters.command(['lq'], PREFIXES))
async def gate_lq(client, message):
    ...
    msgedit = await client.send_message(
        ...
        text=f"""
<b>{symbol} Gateway: Braintree AVS CCN</b> (/lq) 
...
<b>{symbol} Processing Please Wait...</b>.
    ...
    response, status,avs,cvv2 = await b3_gate(cc, mes, ano, cvv, await ProxyRandom())
...
{symbol} AVS: {avs} CVV:{cvv2}
{symbol} Gateway: Nmi <code>[CCN]</code>
