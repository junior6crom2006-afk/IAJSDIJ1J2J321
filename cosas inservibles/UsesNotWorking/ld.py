...
@Client.on_message(filters.command(['ld'], PREFIXES))
async def gate_te(client, message):
    ...
    gate_info = GateManager.get_gate_info("/ld")
...
{symbol} Última Revisión: {gate_info['ultima_revision']}</b>""", reply_to_message_id=message.id)
    ...
    msgedit = await client.send_message(
        ...
        text=f"""
<b>{symbol} Gateway: Eway CCN</b> (/ld) 
...
<b>{symbol} Took: {taken2}</b>
    ...
    while retries < max_retries:
        try:
            ...
            status, response = await eway(cc, mes, ano, cvv, await ProxyRandom())
...
{symbol} Gateway: Eway <code>[CCN]</code>
