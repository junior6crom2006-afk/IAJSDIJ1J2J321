...
from gates.mercadopago import mp
...
@Client.on_message(filters.command(['mp'], PREFIXES))
async def gate_te(client, message):
    ...
    msgedit = await client.send_message(
        ...
        text=f"""
<b>{symbol} Gateway: Mercado Pago 4$ </b> (/mp) 
...
<b>{symbol} Took: {taken2}</b>
    ...
    while retries < max_retries:
        try:
            ...
            status, response = await mp(cc, mes, ano, cvv,)
...
{symbol} Gateway: Mercado Pago <code>[CHARGED]</code>
